...
def _synth_doc():
    return {
        ...
        "source": "instagram",
        "contact": {
            "username": "rachel_123@instagram", "alias": "rachel",
            "nick_name": "Rachel", "remark": "", "display_name": "Rachel",
...
def _export_messages():
    return [
        {"sender_name": "Bou Juan", "timestamp_ms": 1_780_000_002_000,
         "content": _mojibake("Ola Raquelinha 😆"), "is_geoblocked_for_viewer": False,
         "is_unsent_image_by_messenger_kid_parent": False},
        {"sender_name": _mojibake("Rächel 🇲🇴"), "timestamp_ms": 1_780_000_001_000,
         "content": "hey there"},
        {"sender_name": "Bou Juan", "timestamp_ms": 1_780_000_003_000,
         "content": "You sent an attachment.",
         "share": {"link": "https://instagram.com/reel/X",
                   "share_text": _mojibake("look 😎")}},
        {"sender_name": "Bou Juan", "timestamp_ms": 1_780_000_004_000,
         "photos": [{"uri": "…/photos/1.jpg", "creation_timestamp": 1}]},
        {"sender_name": _mojibake("Rächel 🇲🇴"), "timestamp_ms": 1_780_000_005_000,
         "audio_files": [{"uri": "…/audio/1.ogg"}]},
        {"sender_name": "Bou Juan", "timestamp_ms": 1_780_000_006_000,
         "content": "Video chat ended", "call_duration": 42},
        {"sender_name": "Bou Juan", "timestamp_ms": 1_780_000_007_000,
         "content": "deleted", "is_unsent": True},
    ]


def test_normalize_export_messages():
    recs = instagram._normalize_export_messages(_export_messages(), me_name="Bou Juan")
    ...
    assert recs[0]["sender_name"] == "Rächel 🇲🇴"
    text = next(r for r in recs if r["type"] == TYPE_TEXT and "Ola" in r["content"])
    assert text["content"] == "Ola Raquelinha 😆"
    ...
    assert "look 😎" in share["content"] and "instagram.com/reel/X" in share["content"]
    # media + call types
...
def _make_export_zip(tmp_path) -> Path:
    title = _mojibake("Rächel 🇲🇴")
    doc = {
        "participants": [{"name": title}, {"name": "Bou Juan"}],
        "messages": _export_messages(),
        "title": title,
        "is_still_participant": True,
        "thread_path": "inbox/rachel_123",
        "magic_words": [],
    }
    zpath = tmp_path / "export.zip"
    arc = "your_instagram_activity/messages/inbox/rachel_123/message_1.json"
...
def test_fetch_from_export_end_to_end(tmp_path):
    ...
    assert len(rows) == 1
    assert rows[0]["slug"] == "rachel"
    assert rows[0]["display_name"] == "Rächel 🇲🇴"
    # fetch by slug substring → one wxextract-instagram doc
    docs = instagram.fetch_from_export(zpath, "rachel")
    assert len(docs) == 1
    doc = docs[0]
    assert doc["schema"] == "wxextract-instagram/1"
    assert doc["contact"]["alias"] == "rachel"
    assert doc["contact"]["username"] == "rachel_123@instagram"
    assert doc["my_label"] == "Bou Juan"          # auto-derived (participant != title)
    assert doc["contact"]["message_count"] == 6
    # round-trips through the loader and stats
    out = tmp_path / "rachel.json"
    instagram.write_doc(doc, out)
    contact, msgs, _ = instagram.load_instagram_json(out)
    assert len(msgs) == 6
    assert sum(1 for m in msgs if m.is_me) == 4   # Bou Juan sent 4 of the 6
...
def test_derive_me_and_groups():
    assert instagram._derive_me("Rachel", ["Rachel", "Bou Juan"], None) == "Bou Juan"
    assert instagram._derive_me("Rachel", ["Rachel", "Bou Juan"], "Override") == "Override"
